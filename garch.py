...
def calc(c, df):
    ...
    for i in range(2, len(df.index)):
        ...
        df.simulated_var[i] = gamma + alpha * (df.simulated_log_returns[i - 1] - log_mean) ** 2 + (
        beta * df.simulated_var[i - 1])

        df.simulated_AtMVol[i] = df.simulated_var[i] ** .5

        df.simulated_futures[i] = df.simulated_futures[i - 1] * np.exp(log_mean \
                                                                       + (df.stocastic_factor[i] * df.simulated_AtMVol[
            i] / 16))
